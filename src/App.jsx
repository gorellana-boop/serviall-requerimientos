...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try{const r=await window.storage.get("sys_users_v2");setUsers(r?JSON.parse(r.value):DEFAULT_USERS);}
      catch{setUsers(DEFAULT_USERS);}
      setUL(true);
    })();
  },[]);
  useEffect(()=>{if(users&&ul) window.storage.set("sys_users_v2",JSON.stringify(users)).catch(()=>{});},[users,ul]);
...
function Dashboard({ user, users, setUsers, onLogout, onUserUpdated }) {
  ...
  useEffect(()=>{
    (async()=>{
      try{const r=await window.storage.get("reqs_cliente_v3");setDC(r?JSON.parse(r.value):RAW_DATA.map(d=>({...d,stages:[...d.stages]})));}
      catch{setDC(RAW_DATA.map(d=>({...d,stages:[...d.stages]})));}
      try{const r=await window.storage.get("reqs_interno_v3");setDI(r?JSON.parse(r.value):[]);}
      catch{setDI([]);}
      setLoaded(true);
    })();
  },[]);
  useEffect(()=>{if(dataCliente&&loaded) window.storage.set("reqs_cliente_v3",JSON.stringify(dataCliente)).catch(()=>{});},[dataCliente,loaded]);
  useEffect(()=>{if(dataInterno&&loaded) window.storage.set("reqs_interno_v3",JSON.stringify(dataInterno)).catch(()=>{});},[dataInterno,loaded]);
  ...
  useEffect(()=>{
    ...
    const checkDueAlerts = async () => {
      ...
      let sent = {};
      try { const r=await window.storage.get("due_alerts_sent_v1"); sent=r?JSON.parse(r.value):{}; } catch{}
      ...
      if(sentCount>0){
        await window.storage.set("due_alerts_sent_v1",JSON.stringify(sent)).catch(()=>{});

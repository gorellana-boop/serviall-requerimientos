    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:800,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"rgb(40,43,52)",borderRadius:16,padding:"1.75rem",width:"min(520px,94vw)",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,0.3)"}}>
        ...
        <CustomSelect
          ...
          options={responsables.map(r=>({value:r.name,label:`${r.name} (${r.email})`}))}
        />
        {showStages&&<>
...
// ─── CUSTOM SELECT ────────────────────────────────────────────────────────────
function CustomSelect({value, onChange, options, placeholder="— Seleccionar —", style={}}){
  ...
  return(
    <div ref={ref} style={{position:"relative",...style}}>
      <div onClick={()=>setOpen(o=>!o)} style={{...inputStyle,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",userSelect:"none"}}>
        <span style={{color:value?"#fff":"rgba(255,255,255,0.35)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1,minWidth:0}}>{label}</span>
        <span style={{fontSize:10,color:"rgba(255,255,255,0.4)",marginLeft:8,flexShrink:0,transform:open?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s"}}>▼</span>
      </div>
      {open&&(
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"rgb(28,31,40)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,boxShadow:"0 12px 40px rgba(0,0,0,0.6)",zIndex:500,maxHeight:240,overflowY:"auto"}}>
          {placeholder&&<div onClick={()=>{onChange("");setOpen(false);}} style={{padding:"9px 12px",cursor:"pointer",fontSize:13,color:"rgba(255,255,255,0.35)",background:"rgb(28,31,40)",borderBottom:"1px solid rgba(255,255,255,0.06)"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.08)"} onMouseLeave={e=>e.currentTarget.style.background="rgb(28,31,40)"}>— Sin asignar —</div>}
          {options.map((o,i)=>{const v=o.value||o;const l=o.label||o;return(
            <div key={i} onClick={()=>{onChange(v);setOpen(false);}} style={{padding:"9px 12px",cursor:"pointer",fontSize:13,color:v===value?"#60A5FA":"rgba(255,255,255,0.85)",background:v===value?"rgba(37,99,235,0.15)":"rgb(28,31,40)",fontWeight:v===value?600:400}} onMouseEnter={e=>e.currentTarget.style.background=v===value?"rgba(37,99,235,0.2)":"rgba(255,255,255,0.08)"} onMouseLeave={e=>e.currentTarget.style.background=v===value?"rgba(37,99,235,0.15)":"rgb(28,31,40)"}>
              {l}
            </div>
          );})}
        </div>
...
function Dashboard({ user, users, setUsers, onLogout, onUserUpdated }) {
  ...
  const data=activeTab==="cumplidos"
    ? [...(dataCliente||[]),...(dataInterno||[])].filter(d=>pct(d.stages)===100)
    : activeTab==="cliente"?dataCliente:dataInterno;
  const setData=activeTab==="cliente"?setDC:activeTab==="interno"?setDI:()=>{};
  ...
  const filtered=useMemo(()=>{
    ...
    return data.filter(d=>{
      ...
      // En tabs cliente/interno ocultar completados (van a pestaña Cumplidos)
      if(activeTab!=="cumplidos"&&p===100) return false;
    ...
    <div style={{display:"flex",minHeight:"100vh",background:"rgb(30,33,40)",fontFamily:"'Segoe UI',system-ui,sans-serif",fontSize:14,color:"#fff"}}>
      ...
      <aside style={{background:"rgb(22,25,30)",borderRight:"1px solid rgba(255,255,255,0.07)",width:232,flexShrink:0,display:"flex",flexDirection:"column",gap:"1rem",padding:"1.25rem 0.875rem",overflowY:"auto"}}>
        ...
        <div style={{marginTop:"auto",display:"flex",flexDirection:"column",gap:7}}>
          ...
          {can(role,"addItem")&&mainView==="dashboard"&&activeTab!=="cumplidos"&&<button onClick={()=>{setEI(null);setModal(true);}} style={{...btnStyle,background:"#2563EB",color:"#fff",border:"none",justifyContent:"center",width:"100%"}}>+ Agregar ítem</button>}
          {can(role,"addItem")&&mainView==="dashboard"&&activeTab!=="cumplidos"&&<button onClick={()=>setBulkModal(true)} style={{...btnStyle,background:"rgba(37,99,235,0.15)",color:"#60A5FA",border:"1px solid rgba(37,99,235,0.3)",justifyContent:"center",width:"100%"}}>📋 Importar lista</button>}
      ...
      <main style={{flex:1,overflow:"auto",padding:"1.5rem",display:"flex",flexDirection:"column",gap:"1.25rem",minWidth:0}}>
        ...
        {mainView==="dashboard"&&data&&(<>
          ...
          <div style={{display:"flex",gap:4,background:"rgb(22,25,30)",borderRadius:12,padding:4,width:"fit-content"}}>
            {[{id:"cliente",label:"📦 Requerimientos Cliente",color:"#2563EB"},{id:"interno",label:"🔧 Internos Serviall",color:"#0D9488"},{id:"cumplidos",label:"✅ Cumplidos",color:"#16A34A"}].map(tab=>(
              <button key={tab.id} onClick={()=>{setAT(tab.id);setView("all");setSrch("");setSF("");}} style={{padding:"9px 18px",borderRadius:9,border:"none",cursor:"pointer",fontSize:13,fontWeight:600,transition:"all 0.2s",background:activeTab===tab.id?tab.color:"transparent",color:activeTab===tab.id?"#fff":"rgba(255,255,255,0.4)",boxShadow:activeTab===tab.id?"0 2px 8px rgba(0,0,0,0.3)":"none"}}>
                {tab.label}{tab.id==="cumplidos"&&<span style={{marginLeft:6,fontSize:11,background:"rgba(255,255,255,0.2)",borderRadius:20,padding:"1px 7px"}}>{[...(dataCliente||[]),...(dataInterno||[])].filter(d=>pct(d.stages)===100).length}</span>}
              </button>
            ))}
          </div>
